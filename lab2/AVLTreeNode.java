...
public class AVLTreeNode<T extends Comparable<T>> {
	...
	//Kolla om vi nu får obalans!!
	
	calculateHeight(this);
	
	int leftNodeWeight = -1;
	int rightNodeWeight = -1;
	
	if(left != null) {
		 leftNodeWeight =  nodeHeight(left);
	}
	if(right != null) {
		 rightNodeWeight = nodeHeight(right);
	}	
	
	if(Math.abs(rightNodeWeight - leftNodeWeight) > 1) {		//Det är obalans
		
		//Vilket håll är det obalans åt?
